...
public class JwtService {
	...
	public JwtService() {
		try {
			...
			System.out.println("JwtService 생성자를 통해서 만들어진 secretKey : " + secretKey);
	...
	/*
	 * 이름, 만료일 등을 통한 정보 입력
	 * Map으로 생성 후에 Jwts.claims를 통해서 변환
	 * 0.12 이전 버전에서는 setCliam을 통해서 생성
	 */
	...
	public String extractUsername(String token) {
		//jwt에서 다양한 claims 필드에서 사용자 이름을 추출할 수 있도록 작성
	...
	private Claims extractAllClaims(String token) {
		return Jwts.parser()
				.verifyWith(getkey())	//jwt를 검증할 키, 처음만들어 키를 제공ㅎ면 jwt의 유효성을 확인 : 내가만든거니?
				.build()	//파서 객체 빌드
	...
	public boolean validateToken(String token, UserDetails userDetails) {
		...
		//token의 subject 값과 인증 객체(UserDetails)와 같은지 확인 && 만료 시간 확인
