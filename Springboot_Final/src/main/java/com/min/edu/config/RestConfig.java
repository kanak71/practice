...
public class RestConfig implements RepositoryRestConfigurer {
	...
	public void configureRepositoryRestConfiguration(RepositoryRestConfiguration config, CorsRegistry cors) {
		RepositoryRestConfigurer.super
			...
			cors.addMapping("/**") // Spring REST API 의 기본 주소는 application.properties에 정의 "spring.data.rest.base-path=/api" 요청이름 허용
		    .allowedOrigins("http://localhost:5173") // 허용될 출처를 지정 
		    .allowedMethods("GET","POST","PUT","DELETE","PATCH", "OPTIONS") //허용할 HTTP 메소드 요청 설정
		    .allowCredentials(true) // 자격인증 증명(쿠키, 인증 정보 등) 허용 설정
