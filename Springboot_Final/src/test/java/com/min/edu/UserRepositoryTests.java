...
class UserRepositoryTests {
	...
	public void loginSessionTest() throws Exception{
		...
		//요청(form-POST, x-www-form-urlencode로 전송)
		...
		mockMvc.perform(get("/api")
						...
						.session(session)	//로그인된 세션이 상태 정보를 MockMvc 흉내낸다
						.accept(MediaType.APPLICATION_JSON)	//클라이언트가 응답하는 타입 지정 : JSON으로 응답해주세요
						.with(csrf()))	//CSRF토큰 추가
		.andExpect(status().isOk())
		.andDo(document("after login api", 
